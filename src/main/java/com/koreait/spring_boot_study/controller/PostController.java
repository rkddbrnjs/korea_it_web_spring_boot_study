...
//1. controller
//html(웹페이지) 반환하는 서버 사이드 랜더링 (SSR)
//2. RestController
//JSON, 문자열 등 다양한 데이터를 반환 => REST API 서버

//서버 사이드 랜더링은 서버측에서 프론트의 웹페이지까지 통째로 반환 그걸 부라우저에서 띄움
//그럼 해당 요청 경로에 따라서 웹페이지가 다 할당되어있어서 느림

//하지만 우리가 하게 될 프론트(리엑트) + 백엔드(스프링부트 웹서버)
...
//    private final PostService postService;
    ...
    //이미 실행될 때 ioc컨테이너에 객체들이 생성되서 보관되어있다.
